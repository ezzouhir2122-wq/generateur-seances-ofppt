...
function pctBadgeStyle(v: number | undefined): React.CSSProperties {
  ...
  if (v >= 75) return { color: "#39C84A", background: "#39C84A14" };
                  ...
                  {stagiaires.length === 0 ? (
                    <tr>
                      <td colSpan={competences.length + 1} className="text-center py-10" style={{ color: "#4B5563" }}>
                        Aucun stagiaire. <a href={`/suivi/${groupeId}/stagiaires`} className="underline" style={{ color: "#39C84A" }}>Ajouter des stagiaires →</a>
                      </td>
                    </tr>
                  ) : (
                    stagiaires.map((s, i) => (
                      <tr
                        ...
                        style={{
                          background: s.id === selectedId ? "#39C84A10" : i % 2 === 0 ? "#0D0D12" : "#111116",
                          borderBottom: "1px solid #1E1E2C",
                          outline: s.id === selectedId ? "1px solid #39C84A30" : "none",
                        }}
                      >
                        <td className="px-4 py-2.5 font-medium sticky left-0 z-10" style={{ background: "inherit", color: s.id === selectedId ? "#39C84A" : "#E5E7EB" }}>
                          {s.prenom} {s.nom}
                        </td>
                        {competences.map((c) => {
