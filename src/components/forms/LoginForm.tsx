...
export default function LoginForm({ error }: { error?: string }) {
  ...
  const [localError, setLocalError] = useState<string | null>(
    error ? ERROR_MESSAGES[error] ?? "Email ou mot de passe incorrect" : null
  );
  const [showPassword, setShowPassword] = useState(false);
  const [resetOk, setResetOk] = useState(false);

  useEffect(() => {
    if (typeof window !== "undefined" && new URLSearchParams(window.location.search).get("reset") === "ok") {
      setResetOk(true);
    }
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (result?.error) {
      setLocalError(ERROR_MESSAGES[result.error] ?? "Email ou mot de passe incorrect");
      setIsLoading(false);
    } else {
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            {showPassword ? (
              <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                <path d="M17.94 17.94A10.07 10.07 0 0112 20C5 20 1 12 1 12a18.45 18.45 0 015.06-5.94M9.9 4.24A9.12 9.12 0 0112 4c7 0 11 8 11 8a18.5 18.5 0 01-2.16 3.19" />
      ...
      <div className="flex items-center justify-end pt-1">
        <Link
          ...
          className="text-xs font-medium text-[#0A4DA8] hover:underline"
        >
          Mot de passe oublié ?
        </Link>
      </div>
