...
export async function GET() {
  ...
  const [pending, approved, suspended, rejected] = await Promise.all([
    ...
    prisma.user.findMany({
      where: { status: "SUSPENDED", role: "FORMATEUR" },
      select,
      orderBy: { approvedAt: "desc" },
    }),
    prisma.user.findMany({
  ...
  return NextResponse.json({ pending, approved, suspended, rejected });
}
