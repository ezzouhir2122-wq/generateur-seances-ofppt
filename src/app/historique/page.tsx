        ...
        <div className="grid gap-3">
          {seances.map((s) => (
            <Link
              ...
              onMouseEnter={e => ((e.currentTarget as HTMLAnchorElement).style.borderColor = "#84CC1640")}
              onMouseLeave={e => ((e.currentTarget as HTMLAnchorElement).style.borderColor = "#1E1E2C")}
            >
              <div className="flex items-start gap-4">
                <div
                  className="w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm shrink-0"
                  style={{ background: "#84CC1614", color: "#84CC16" }}
                >
                  {s.filiere.slice(0, 2).toUpperCase()}
                </div>
                <div>
                  <p className="font-semibold text-white group-hover:text-[#84CC16] transition-colors">
                    {s.title}
                  </p>
